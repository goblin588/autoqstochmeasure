...
def tune_delays(
    ...
    with Logic16(coincidence_window=windows[0], logic_mode=True,
        ...
        try:
            ...
            interrupted = True
            print("\nInterrupted — showing delays found for channels already tuned.")
    ...
    if background is not None:
        int_desc = f"{integration_time:.2g}s" if integration_time is not None else "per-channel default"
        print(f"Background used for minimum-count bar: {background:.1f} counts/{int_desc} "
              f"(avg of {background_samples} samples, "
    ...
    if not changes:
        print("Nothing tuned yet — nothing to save.")
        return
    prompt = "Save the delays found so far as new defaults? [y/N] " if interrupted \
        else "Save tuned delays as new defaults? [y/N] "
...
def scan_and_record(
    ...
    with Logic16(coincidence_window=COINCIDENCE_WINDOW, logic_mode=True,
        ...
        logic.set_delays(delays)
        logic.set_integration_window(record_bin_s)
...
def measure_background(
    ...
    `n_bins` reads of `bin_s` seconds each (default 20 x 5s = 100s) at
    `peak_delay + offset` (default +15ns — same BACKGROUND_OFFSET_NS
    convention as calibrate_background/tune_delays), far enough off the
    real coincidence peak found by scan_and_record to miss it and count
    only accidentals.
